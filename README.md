...
Enterprise 授权可绑定到具体机器：**换设备后授权自动失效**，防止授权文件被随意复制扩散。授权通过管理页面上传激活。

```bash
# 1. 在目标机器上取本机机器码（无需授权即可运行）
./neuralgate-enterprise machine-id
# 输出一串机器码，也可在管理页「系统信息 → 授权信息 → 本机机器码」处复制

# 2. 将机器码提供给供应商，签发绑定该机器的授权文件
```

3. 在管理页「系统信息 → 授权信息 → 上传授权文件」上传授权文件（上传时即校验签名、有效期与机器码，并回显结果）
4. **重启服务**后企业功能生效

说明：

- 授权文件的读取路径由 `license.file_path` 指定，上传成功后写入该路径
- 授权缺失、过期或机器码与本机不匹配时，自动降级为 OSS 模式运行
- 授权过期或更换机器后，管理员仍可进入后台取码并上传新授权修复
